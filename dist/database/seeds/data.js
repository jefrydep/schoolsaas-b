...
const EVALUATION_TYPES = [evaluation_type_enum_1.EvaluationType.EXAM, evaluation_type_enum_1.EvaluationType.QUIZ, evaluation_type_enum_1.EvaluationType.PROJECT, evaluation_type_enum_1.EvaluationType.HOMEWORK];
...
function generateSeedData() {
    ...
    const tenantTypeMap = {
        'sanmarin': tenant_type_enum_1.TenantType.INSTITUCION,
        'cnulima': tenant_type_enum_1.TenantType.INSTITUCION,
        'uedue': tenant_type_enum_1.TenantType.INSTITUCION,
        'itn': tenant_type_enum_1.TenantType.INSTITUTO,
    };
    const tenantLevelMap = {
        'sanmarin': tenant_type_enum_1.TenantLevel.SECUNDARIA,
        'cnulima': tenant_type_enum_1.TenantLevel.SECUNDARIA,
        'uedue': tenant_type_enum_1.TenantLevel.PRIMARIA,
        'itn': tenant_type_enum_1.TenantLevel.SUPERIOR,
    };
    ...
    for (let i = 0; i < TENANT_COUNT; i++) {
        ...
        const tenant = {
            ...
            ruc: faker_1.faker.string.numeric(11),
            modularCode: `MOD${faker_1.faker.string.numeric(3)}`,
            ...
            directorName: faker_1.faker.person.fullName(),
            isActive: true,
    ...
    for (let tenantIdx = 0; tenantIdx < TENANT_COUNT; tenantIdx++) {
        for (let j = 0; j < TEACHERS_PER_TENANT; j++) {
            ...
            teachers.push({
                ...
                hireDate: faker_1.faker.date.past({ years: 10 }).toISOString().split('T')[0],
                isActive: true,
    ...
    for (let tenantIdx = 0; tenantIdx < TENANT_COUNT; tenantIdx++) {
        ...
        for (let j = 0; j < STUDENTS_PER_TENANT; j++) {
            ...
            students.push({
                ...
                dateOfBirth: faker_1.faker.date.between({ from: '2005-01-01', to: '2012-12-31' }).toISOString().split('T')[0],
                ...
                documentNumber: isPeru ? faker_1.faker.string.numeric(8) : faker_1.faker.string.numeric(10),
                ...
                previousSchool: `${faker_1.faker.company.name()} ${faker_1.faker.location.city()}`,
                enrollmentDate: faker_1.faker.date.past({ years: 1 }).toISOString().split('T')[0],
                guardianName: faker_1.faker.person.fullName(),
                guardianDocumentNumber: isPeru ? faker_1.faker.string.numeric(8) : faker_1.faker.string.numeric(10),
                relationship: ['Padre', 'Madre', 'Tutor'].pop(),
                occupation: faker_1.faker.person.jobTitle(),
                guardianPhone: faker_1.faker.phone.number(),
                guardianEmail: faker_1.faker.internet.email().toLowerCase(),
                bloodType: BLOOD_TYPES[Math.floor(Math.random() * BLOOD_TYPES.length)],
                allergies: Math.random() > 0.7 ? faker_1.faker.lorem.sentence() : null,
                diseases: Math.random() > 0.8 ? faker_1.faker.lorem.sentence() : null,
                disability: Math.random() > 0.9 ? faker_1.faker.lorem.sentence() : null,
    ...
    for (let courseIdx = 0; courseIdx < courses.length; courseIdx++) {
        for (let j = 0; j < EVALUATIONS_PER_COURSE; j++) {
            ...
            evaluations.push({
                ...
                date: faker_1.faker.date.between({ from: '2026-03-01', to: '2026-06-30' }).toISOString().split('T')[0],
                isActive: true,
...
exports.seedData = generateSeedData();
