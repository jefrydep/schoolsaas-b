...
let AdminsService = class AdminsService {
    ...
    async create(dto) {
        if (!dto.tenantId) {
            throw new common_1.ForbiddenException('tenantId is required to create an admin');
        }
    ...
    async findOne(id) {
        const admin = await this.userRepository.findOne({
            where: { id, role: role_enum_1.Role.ADMIN },
        });
        if (!admin) {
    ...
    async findByTenant(tenantId) {
        return this.userRepository.find({
            where: { tenantId, role: role_enum_1.Role.ADMIN },
            order: { createdAt: 'DESC' },
        });
    }
};
